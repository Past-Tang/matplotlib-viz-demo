...
# 筛选需要的列
data_filtered = data[['姓名', '日期', '签到时间']]

# 转换日期和签退时间为datetime格式
data_filtered['日期'] = pd.to_datetime(data_filtered['日期'])
data_filtered['签退时间'] = pd.to_datetime(data_filtered['签到时间'], format='%H:%M', errors='coerce').dt.time
...
if __name__ == '__main__':
    ...
    plt.ylabel('上班打卡时间', fontsize=14)
    plt.title('电子信息与自动化学院教师上班班打卡时间表', fontsize=34)
